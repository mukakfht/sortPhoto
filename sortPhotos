...
# 创建文件夹并移动文件
def move_file_to_month_folder(filename, taken_date):
    # 使用年份和月份作为文件夹名称
    folder_name = taken_date.strftime("%Y-%m")
    ...
    while os.path.exists(new_path):
        new_name = taken_date.strftime(f"{taken_date.year}-%m-%d_%H-%M-%S") + f"_{i}" + os.path.splitext(filename)[1].lower()
...
for filename in os.listdir(source_folder):
    ...
    if taken_date:
        # 将文件按年月分类并移动
        move_file_to_month_folder(filename, taken_date)
    else:
